...
class star_data_params:
    ...
    def set_stock_minute_start_end_count(self,minute_start=1,minute_end=242,minute_count=242):
        '''设置回测时，每天第几分钟开始，第几分钟结束'''
...
class star_data_core_time_tradedays_series:
    '''交易日历列表'''
    def __init__(self):
        tradedays_path='/Users/chenzongwei/pythoncode/数据库/日频数据/TradingDate_Daily.mat'
    ...
    def set_backtest_period(self,start,end):
        ...
        self.tradedays_daily=self.tradedays.to_frame(name='date').set_index('date')
        #转化为迭代器形式
        self.date_convert()
        self.tradedays_copy=copy.deepcopy(self.tradedays)
        self.tradedays_total=list(self.tradedays_copy)
        print('本次回测将总共进行{}次迭代'.format(len(self.tradedays_total)))
...
class star_data_core_pri_data_and_iter_entry_dates(star_data_params,star_data_core_time_tradedays_series):
    ...
    def find_and_read_single_pri_data(self,code,files,start,end):
        ...
        file='/'.join(['/Users/chenzongwei/pythoncode/数据库/分钟数据',file])
    ...
    def read_in_all_pri_data(self):
        '''根据股票池读入原始数据'''
        pri_data_files=os.listdir('/Users/chenzongwei/pythoncode/数据库/分钟数据')
        for stock in tqdm.tqdm(self.stock_list,desc='数据加载中'):
...
class star_data_core_iter_data(star_data_core_pri_data_and_iter_entry_dates):
    ...
    def update_iter_data(self,now):
        ...
        if now.minute_order==self.minute_start:
...
class star_data_now_basic_info(star_data_core_iter_data):
    ...
    def update_minute_basic(self,now):
        ...
        if now.minute_order==self.minute_end:
...
class star_data_now_positions:
    ...
    def update_now_positions(self,now,now_data,minute_end,amount=None):
        '''更新相关数据，每个时刻都更新'''
        ...
        if now.minute_order==minute_end:
...
class star_data_single_order(star_data_params):
    ...
    def __init__(
        ...
        if order_life is not None:
            ...
            self.order_life=self.minute_count-now.minute_order
        #订单的动作，买入还是卖出
    ...
    def order_success(self,now,way,now_data,now_cash,now_cap,positions,total_sell_today):
        ...
        if self.order_action=='sell':
            left_amount=max((positions[self.order_code].get_last_amount()-total_sell_today[self.order_code]),0)
    ...
    def get_last_close(self,last_closes,code):
        '''获取上一交易日收盘价'''
        try:
            return last_closes[code]
        except Exception:
            return 0
    ...
    def match_orders(self,now,now_data,last_closes,now_cash,now_cap,positions,total_sell_today):
        ...
        if self.order_life<0:
            ...
            if self.order_action=='buy':
                #判断是否涨停
                if now_data[self.order_code].low<self.get_last_close(last_closes,self.order_code)*1.098:
                    #限价买单，当前价格偏低时，可以成交
                ...
                if positions[self.order_code].hold_amount>0:
                    ...
                    #判断是否跌停
                    if now_data[self.order_code].high>=self.get_last_close(last_closes,self.order_code)*0.902:
                        if self.order_method=='limit':
...
class star_comments_and_plot:
    ...
    def get_net_series(self):
        ...
        self.net_series.date=self.net_series.date.apply(lambda x:x.to_datetime())
        self.net_series=self.net_series.set_index(['date'])
        self.net_series=self.net_series.resample('D').last()
        self.net_series=pd.concat([self.net_series,self.moon.tradedays_daily],axis=1).dropna()
...
class star(star_user_function):
    ...
    def run(self,html_name='策略评价.html',actual_iter_tradedays=None):
        '''每个时刻迭代运行的部分'''
        ...
        if not actual_iter_tradedays:
            actual_iter_tradedays=self.tradedays
        for now in tqdm.tqdm(self.tradedays_total):
            if now in actual_iter_tradedays:
                #更新当前时刻
                self.now=now
                #清零当日累积卖出量
                if now.minute_order==self.minute_end:
                ...
                if self.breathing_orders_buy or self.breathing_orders_sell:
                    #更新当前时刻持仓数据
                    for k in self.positions.keys():
                        self.positions[k].update_now_positions(now,self.now_data,self.minute_end)
